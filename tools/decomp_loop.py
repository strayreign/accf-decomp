...
# Ensure project root is on sys.path so `from tools.X` imports work
...
def load_context(unit_name: str, asm_text: str) -> str:
    ...
    Hierarchy (most specific/confident -> least):
      ...
      T1b  Ghidra decompilation        -- pseudocode from main.dol.c (ACCFiles)
      T2   Cross-game symbol matches   -- ACGC-first (same AC lineage), then EGG/Brawl/etc.
      T3   SDK/framework headers       -- official RVL_SDK (ACCFiles) + scraped GitHub
    ...
    # -- T1b: Ghidra decompilation from ACCFiles/main.dol.c ---------------
    t1b = []
    try:
        from tools.accfiles_bridge import get_ghidra_context
        ghidra_ctx = get_ghidra_context(unit_name, asm_text, max_chars=4000)
        if ghidra_ctx:
            t1b.append(ghidra_ctx)
    except Exception:
        pass
    ...
    #    Prefer official RVL_SDK from ACCFiles, fall back to scraped GitHub
    t3 = []
    try:
        from tools.accfiles_bridge import get_relevant_sdk_headers
        sdk_ctx = get_relevant_sdk_headers(asm_text, max_chars=3000)
        if sdk_ctx:
            t3.append(sdk_ctx)
    except Exception:
        pass
    if not t3 and _rs_mod is not None:
        try:
    ...
    if dwc_ctx:
        t5.append(dwc_ctx)
    cf_id_ctx = _load_cf_id_context(asm_text)
    if cf_id_ctx:
        t5.append(cf_id_ctx)
    ...
    TIER_CAPS = [
        ...
        (t1b, 4000),  # T1b Ghidra decomp       -- pseudocode reference
        (t2,  4000),  # T2  cross-game symbols  -- strongest naming hints
        (t3,  3000),  # T3  SDK headers (official RVL_SDK or scraped)
    ...
    TOTAL_BUDGET = 26000   # ~6.5k tokens (bumped for Ghidra tier)
    used  = 0
    parts = []
...
# ── CF item/villager ID lookup ────────────────────────────────────────────────
# Source: ACSE (Cuyler36/ACSE) CF_Items_en.txt + CF_Villagers_en.txt
# Scans assembly for immediate constants that match known CF item/villager IDs.

_CF_ID_TABLE_CACHE: dict | None = None

def _get_cf_id_table() -> dict:
    global _CF_ID_TABLE_CACHE
    if _CF_ID_TABLE_CACHE is None:
        cf_path = DATA_DIR / "scraped" / "cf_id_tables.json"
        if cf_path.exists():
            try:
                import json as _json
                _CF_ID_TABLE_CACHE = _json.loads(cf_path.read_text())
            except Exception:
                _CF_ID_TABLE_CACHE = {}
        else:
            _CF_ID_TABLE_CACHE = {}
    return _CF_ID_TABLE_CACHE


def _load_cf_id_context(asm_text: str) -> str:
    """
    Scan assembly for immediate values that match CF item or villager IDs.
    Returns an annotation block so the LLM can name constants it sees.
    """
    tbl = _get_cf_id_table()
    if not tbl:
        return ""

    items    = tbl.get("items", {})
    villagers = tbl.get("villagers", {})

    # Extract all 16-bit hex immediates from the assembly (li, cmplwi, cmpwi, etc.)
    found_items: list[str] = []
    found_villagers: list[str] = []

    seen: set[str] = set()
    for raw in re.findall(r"0x([0-9A-Fa-f]{3,4})\b", asm_text):
        key = f"0x{int(raw, 16):04X}"
        if key in seen:
            continue
        seen.add(key)
        if key in items:
            found_items.append(f"  {key} = {items[key]}")
        if key in villagers:
            found_villagers.append(f"  {key} = {villagers[key]}")

    if not found_items and not found_villagers:
        return ""

    lines = ["=== CF item/villager ID constants found in this function ==="]
    if found_items:
        lines.append("Items:")
        lines.extend(found_items[:20])
    if found_villagers:
        lines.append("Villagers:")
        lines.extend(found_villagers[:10])
    return "\n".join(lines)
...
def _ghidra_decompile(addr_hex: str) -> str:
    """
    Ask the preferred decompiler server to decompile one function.
    Returns pseudo-C string or '' if not available.
    """
    base = _ghidra_base()
    if not base:
        return ""
    return _decompile_from_url(base, addr_hex)


def _decompile_from_url(base_url: str, addr_hex: str) -> str:
    """
    Query a specific decompiler server (IDA or Ghidra) at base_url for
    the function at addr_hex.  Returns pseudo-C or '' on failure.
    """
    try:
        import httpx
        for addr_param in (f"0x{addr_hex}", addr_hex):
            try:
                r = httpx.get(f"{base_url}/decompile",
...
def _server_reachable(base_url: str) -> bool:
    """Return True if the server at base_url responds to /ping or /list_functions."""
    try:
        import httpx
        r = httpx.get(f"{base_url}/ping", timeout=1.5)
...
def load_ghidra_context(unit_name: str, asm_text: str) -> str:
    """
    Pull pseudo-C from BOTH IDA Hex-Rays (port 8081) and Ghidra (port 8080)
    for the unit's functions (up to 3 each).  Both references are included in
    the prompt so the LLM can cross-reference the two analyses — IDA gives
    Hex-Rays-quality types, Ghidra gives structural hints from a different
    analysis engine.  A cross-reference note is prepended when both are present.
    """
    ...
    _DECOMPILER_SOURCES = [
        ("IDA Hex-Rays", "http://localhost:8081"),
        ("Ghidra",       "http://localhost:8080"),
    ]

    sections = []
    sources_found = []

    for label, base_url in _DECOMPILER_SOURCES:
        if not _server_reachable(base_url):
            continue
        parts = []
        for addr in addrs[:3]:
            code = _decompile_from_url(base_url, addr)
            if code:
                # Strip noise comments, keep function body
                code = re.sub(r'/\*.*?\*/', '', code, flags=re.DOTALL).strip()
                if len(code) > 80:
                    parts.append(f"/* fn_{addr} ({label}) — structural reference only */\n{code}")
        if parts:
            sections.append(
                f"=== {label} reference (structure hint, may not compile) ===\n"
                + "\n\n".join(parts)
            )
    ...
    # Prepend a cross-reference note when both decompilers contributed
    if len(sources_found) >= 2:
        note = (
            "=== Cross-decompiler note ===\n"
            "Both IDA Hex-Rays and Ghidra pseudo-C are provided below.\n"
            "Where they agree on structure/types, that is high-confidence.\n"
            "Where they differ, prefer IDA for type accuracy; "
            "use Ghidra for control-flow structure.\n"
        )
...
def _council_strategy(unit_name: str, asm: str, ctx: str) -> str:
    ...
    # Pull a brief peek at both decompilers to give the council more to work with.
    # This is capped at 600 chars per source so it doesn't swamp the 7B context.
    addrs = re.findall(r"^\.fn\s+fn_([0-9A-Fa-f]{8})", asm, re.MULTILINE)
    _decompiler_snippets = []
    for _label, _port in (("IDA Hex-Rays", "8081"), ("Ghidra", "8080")):
        _base = f"http://localhost:{_port}"
        if addrs and _server_reachable(_base):
            _code = _decompile_from_url(_base, addrs[0])
            if _code and len(_code) > 40:
                _snippet = re.sub(r'/\*.*?\*/', '', _code, flags=re.DOTALL).strip()[:600]
                _decompiler_snippets.append(f"--- {_label} pseudo-C (fn_{addrs[0]}) ---\n{_snippet}")
    _decompiler_block = ("\n\n".join(_decompiler_snippets) + "\n\n") if _decompiler_snippets else ""

    strategy_prompt = (
        f"Analyze this PowerPC assembly for unit '{unit_name}' and give a brief "
        "strategy note for a stronger decompilation model — NOT code, just observations.\n\n"
        + (f"{ctx_head}\n\n" if ctx_head else "")
        + (_decompiler_block)
        + f"=== Assembly (excerpt) ===\n{asm_head}\n\n"
        "Cross-reference the decompiler pseudo-C above (if present) with the assembly.\n"
        "Answer ONLY the following in short bullet points (200 words max):\n"
        ...
        "6. Do IDA and Ghidra agree on the structure? Note any disagreements.\n"
        "7. Any pattern that would trip up a decompiler (loop, switch, inline fn)?\n"
...
def process_large_unit(
    ...
    if len(functions) >= 3:
        ...
        if strategy:
            # Strip the section header and print a brief excerpt so the user can
            # see what the council actually found — not just "strategy ready".
            _preview = strategy
            for _hdr in ("=== Pre-analysis (7B strategy council — use as structural guide) ===\n",
                         "=== Pre-analysis ===\n"):
                _preview = _preview.replace(_hdr, "")
            _lines = [l for l in _preview.strip().splitlines() if l.strip()]
            print("  💡  Council findings:")
            for _ln in _lines[:10]:
                print(f"       {_ln.strip()}")
            if len(_lines) > 10:
                print(f"       ... ({len(_lines)} lines total — full analysis injected into prompt)")
    ...
    for attempt in range(max_attempts):
        ...
        if attempt == 0:
            ...
            if council_strategy_text:
                # Print what the council actually found so the operator can see
                # the models are doing real work, not just silently passing a blob.
                _preview = council_strategy_text
                for _hdr in ("=== Pre-analysis (7B strategy council \u2014 use as structural guide) ===\n",
                             "=== Pre-analysis ===\n"):
                    _preview = _preview.replace(_hdr, "")
                _lines = [l for l in _preview.strip().splitlines() if l.strip()]
                print("  \U0001f4a1  Council findings:")
                for _ln in _lines[:10]:
                    print(f"       {_ln.strip()}")
                if len(_lines) > 10:
                    print(f"       ... ({len(_lines)} lines \u2014 injecting full analysis into prompt")
