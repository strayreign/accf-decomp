...
Models (cheapest → strongest):
  0: Qwen2.5-Coder 7B  — fast free local model, first try
  1: CodeGemma 7B      — free alternative if Qwen fails/times out
  2: Claude Haiku      — free via proxy (Qwen), or paid fallback
  3: Claude Sonnet     — free via proxy (Qwen), or paid last resort
...
if _PROFILE == "windows":
    ...
    MODELS = [
        ...
        {"id": "haiku",      "backend": "claude",
         ...
         "label": "Claude Haiku         (→ 30B via proxy)" if _via_proxy else "Claude Haiku         (paid)"},
        {"id": "sonnet",     "backend": "claude",
         "name":  "claude-sonnet-4-6",
         "label": "Claude Sonnet        (→ 30B via proxy)" if _via_proxy else "Claude Sonnet        (paid)"},
    ]
else:
...
def call_claude(model_name: str, prompt: str) -> str:
    ...
    # Support free-claude-code proxy: if ANTHROPIC_BASE_URL is set, route through it.
    # The proxy accepts any API key; use ANTHROPIC_AUTH_TOKEN if set, else "freecc".
    base_url = os.environ.get("ANTHROPIC_BASE_URL")
    if base_url:
        key = os.environ.get("ANTHROPIC_AUTH_TOKEN") or os.environ.get("CLAUDE_API_KEY") or "freecc"
        print(f"  🔀  Using proxy: {base_url}", flush=True)
    else:
...
def generate_c(level: int, prompt: str,
    ...
    try:
        if m["backend"] == "ollama":
            result = call_ollama(m["name"], prompt, url=effective_url)
        else:
