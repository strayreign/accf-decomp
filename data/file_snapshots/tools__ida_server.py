...
def _setup_dol_manually() -> bool:
    ...
    # ── Full auto-analysis ────────────────────────────────────────────────────
    _log("[ida_server] Running full auto-analysis (may take several minutes) ...")

    # Use ida_auto.plan_and_wait on every text segment — this is the most
    # reliable way to force IDA to analyse a range when the processor was
    # just configured or changed.  Falls back to auto_mark_range if unavailable.
    for i in range(7):
        if text_size[i] and text_addr[i]:
            start = text_addr[i]
            end   = start + text_size[i]
            if _ida_auto is not None:
                try:
                    _ida_auto.plan_and_wait(start, end)
                    _log(f"[ida_server] plan_and_wait Text{i}: 0x{start:08X}–0x{end:08X}")
                    continue
                except Exception as _pe:
                    _log(f"[ida_server] plan_and_wait failed ({_pe}), falling back")
            idaapi.auto_mark_range(start, end, idaapi.AU_CODE)
            idaapi.auto_mark_range(start, end, idaapi.AU_FINAL)

    _log("[ida_server] auto_wait() — blocking until queue empty ...")
    idaapi.auto_wait()

    n = sum(1 for _ in idautils.Functions())
    _log(f"[ida_server] Manual setup complete — {n} function(s) found")
    return n > 0
...
if seg_count == 0:
    ...
    # DOL loader activated (segments exist) but analysis was suppressed or
    # hasn't run yet.  Run targeted plan_and_wait on every text segment so
    # IDA discovers functions — same approach as _setup_dol_manually but we
    # skip the manual segment/byte setup since the loader already did it.
    _log(
        f"[ida_server] DOL loader OK ({seg_count} segs) but 0 functions. "
        "Running targeted analysis on text segments ..."
    )
    _analysed = 0
    for _seg_ea in idautils.Segments():
        ...
        # Only analyse CODE/TEXT segments — skip DATA/BSS
        if _seg.type not in (ida_segment.SEG_CODE,):
            # also catch segments named Text* even if type is unknown
            if not (_seg_name or "").startswith("Text"):
                continue
        _start = _seg.start_ea
        _end   = _seg.end_ea
        _log(f"[ida_server] plan_and_wait {_seg_name}: 0x{_start:08X}–0x{_end:08X}")
        try:
            if _ida_auto is not None:
                _ida_auto.plan_and_wait(_start, _end)
            else:
            ...
            _log(f"[ida_server] plan_and_wait failed for {_seg_name}: {_ae}")
    # Final flush after all ranges queued
    _log("[ida_server] Final auto_wait() after targeted analysis ...")
    idaapi.auto_wait()
    func_count = sum(1 for _ in idautils.Functions())
    _log(f"[ida_server] Targeted analysis complete — {func_count} function(s) found "
         f"across {_analysed} segment(s)")
else:
    _log(
...
# This keeps IDA's event loop alive indefinitely.
